...
import { nodeToString, generateArrowDecorator, generateNamedImports, generateObjectLiteralDecorator, generateIndexExport } from "./utils";
...
export class ModelBuilder extends Builder {
  ...
  private static buildColumnPropertyDecl(col: IColumnMetadata, dialect: Dialect): ts.PropertyDeclaration {
    ...
    return ts.createProperty([...(col.foreignKey ? [generateArrowDecorator(foreignKeyDecorator, [col.foreignKey.targetModel])] : []), generateObjectLiteralDecorator("Column", buildColumnDecoratorProps(col)), ...(col.indices && col.indices.length ? col.indices.map((index) => generateObjectLiteralDecorator("Index", buildIndexDecoratorProps(index))) : [])], undefined, col.name, col.autoIncrement || col.allowNull ? ts.createToken(ts.SyntaxKind.QuestionToken) : ts.createToken(ts.SyntaxKind.ExclamationToken), ts.createTypeReferenceNode(dialect.mapDbTypeToJs(col.type) ?? "any", undefined), undefined);
  }
  ...
  private static buildAssociationPropertyDecl(association: IAssociationMetadata): ts.PropertyDeclaration {
    ...
    return ts.createProperty([...(association.sourceKey ? [generateArrowDecorator(associationName, targetModels, { sourceKey: association.sourceKey })] : [generateArrowDecorator(associationName, targetModels)])], undefined, associationName.includes("Many") ? pluralize.plural(targetModel) : pluralize.singular(targetModel), ts.createToken(ts.SyntaxKind.QuestionToken), associationName.includes("Many") ? ts.createArrayTypeNode(ts.createTypeReferenceNode(targetModel, undefined)) : ts.createTypeReferenceNode(targetModel, undefined), undefined);
  }
   ...
   */
  private static buildTableClassDeclaration(tableMetadata: ITableMetadata, dialect: Dialect, strict: boolean = true): string {
    const { originName: tableName, name, columns } = tableMetadata;

    let generatedCode = "";

    // Named imports from sequelize-typescript
    generatedCode += nodeToString(generateNamedImports(["Model", "Table", "Column", "DataType", "Index", "Sequelize", foreignKeyDecorator, ...new Set(tableMetadata.associations?.map((a) => a.associationName))], "sequelize-typescript"));
    ...
    [...importModels].forEach((modelName) => {
      generatedCode += nodeToString(generateNamedImports([modelName], `./${modelName}`));
    ...
    if (strict) {
      ...
      const attributesInterface = ts.createInterfaceDeclaration(undefined, undefined, ts.createIdentifier(attributesInterfaceName), undefined, undefined, [...Object.values(columns).map((c) => ts.createPropertySignature(undefined, ts.createIdentifier(c.name), c.autoIncrement || c.allowNull ? ts.createToken(ts.SyntaxKind.QuestionToken) : undefined, ts.createTypeReferenceNode(dialect.mapDbTypeToJs(c.type) ?? "any", undefined)))]);

      generatedCode += "export " + nodeToString(attributesInterface);
      generatedCode += "\n";
    }
    ...
    const classDecl = ts.createClassDeclaration(
      ...
      name,
      undefined,
      !strict ? [ts.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [ts.createExpressionWithTypeArguments([], ts.createIdentifier("Model"))])] : [ts.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [ts.createExpressionWithTypeArguments([ts.createTypeReferenceNode(ts.createIdentifier(attributesInterfaceName), undefined), ts.createTypeReferenceNode(ts.createIdentifier(attributesInterfaceName), undefined)], ts.createIdentifier("Model"))]), ts.createHeritageClause(ts.SyntaxKind.ImplementsKeyword, [ts.createExpressionWithTypeArguments(undefined, ts.createIdentifier(attributesInterfaceName))])],
      // Class members
      [...Object.values(columns).map((col) => this.buildColumnPropertyDecl(col, dialect)), ...(tableMetadata.associations && tableMetadata.associations.length ? tableMetadata.associations.map((a) => this.buildAssociationPropertyDecl(a)) : [])]
    );
